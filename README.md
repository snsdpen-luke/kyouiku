...
| **進路シミュレーション** | 「こんな暮らし」を選んで必要な手取りを逆算し、ハローワークの求人票（高卒）を欄ごとにタップして読み方を学ぶ | 進路学習（高2） |
...
| `regression-shinro.js` | 進路シミュレーションのリグレッション（jsdom・106項目） |
...
node regression-shinro.js 進路シミュレーション.html       # 106項目すべて合格すればOK
```
...
画面右上の版表示（`版 2026-09-18c`）も更新すること。
