...
| **進路シミュレーション** | 「こんな暮らし」を選んで必要な手取りを逆算し、**ハローワークの求人票（見本）と同じ形**の求人票を欄ごとにタップして読み方を学ぶ | 進路学習（高2） |
| **複線図れんしゅう** | 第二種電気工事士 技能試験の候補問題を、単線図から複線図に描き直す。単線図を見て部品を置き、タップで結線し、スイッチを操作して確かめ、部品・結線・電線の色を判定する | 電気工事士の受験指導 |
...
| 進路シミュレーション | <https://snsdpen-luke.github.io/kyouiku/shinro/> |
| 複線図れんしゅう | <https://snsdpen-luke.github.io/kyouiku/fukusen/> |
...
| `進路シミュレーション.html` | **進路シミュレーションの本体。直すのはここだけ** |
| `複線図練習.html` | **複線図れんしゅうの本体。直すのはここだけ** |
| `index.html` | GitHub Pages が配るもの（ラダー工房）。`build.py` が作る**生成物**。手で編集しない |
| `shinro/index.html` | GitHub Pages が配るもの（進路シミュレーション）。同じく**生成物** |
| `fukusen/index.html` | GitHub Pages が配るもの（複線図れんしゅう）。同じく**生成物** |
...
| `HANDOFF-複線図練習.md` | **複線図れんしゅうの引き継ぎ書。問題データの足し方もここ** |
| `HANDOFF-進路シミュレーション.md` | **進路シミュレーションの引き継ぎ書。税率の更新手順もここ** |
...
node regression-shinro.js 進路シミュレーション.html       # 121項目すべて合格すればOK
node regression-fukusen.js 複線図練習.html          # 「すべて合格」と出ればOK
```
...
node regression-shinro.js 進路シミュレーション.html       #    直したほうだけでよい
python3 build.py                                  # 2. 配布物を作り直す
git add -A && git commit -m "…"                   # 3. 記録
git push                                          # 4. 生徒の画面に反映される
```

画面右上の版表示（`版 2026-09-18d`）も更新すること。
